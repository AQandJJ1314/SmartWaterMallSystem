...
import com.atcode.watermall.watermallsearch.feign.ProductFeignService;
import com.atcode.watermall.watermallsearch.service.MallSearchService;
import com.atcode.watermall.watermallsearch.vo.AttrResponseVo;
...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult getSearchResult(SearchParam searchParam) {
        ...
        try {
            SearchResponse searchResponse = restHighLevelClient.search(request, WatermallElasticSearchConfig.COMMON_OPTIONS);
    ...
    private SearchResult bulidSearchResult(SearchParam searchParam, SearchResponse searchResponse) {
        ...
        if (attrs != null && attrs.size() > 0) {
            List<SearchResult.NavVo> navVos = attrs.stream().map(attr -> {
                ...
                navVo.setLink("http://search.watermall.com/list.html" + (replace.isEmpty()?"":"?"+replace));
                return navVo;
